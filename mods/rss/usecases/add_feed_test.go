...
func TestAddFeed_Exec_adds_a_new_feed_to_the_repository(t *testing.T) {
	...
	response, err := addFeed.Add(AddFeedRequest{Name: "foo", Url: "http://localhost.rss"})

	assert.Nil(t, err)
	assert.Equal(t, "UserFeed saved.", response.Message)
...
func TestAddFeed_Exec_handles_repository_errors(t *testing.T) {
	...
	response, err := addFeed.Add(AddFeedRequest{})
